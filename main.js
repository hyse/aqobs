...
async function applicationMain() {
    try {
        ...
        // 记录上一次渲染时是否处于视口裁剪模式（> 5.5）
        let lastWasCull = map.getZoom() > 5.5;
        map.on('moveend', () => {
            pushStateToURL();
            const currentZoom = map.getZoom();
            const isCull = currentZoom > 5.5;
            // 仅在以下两种情况才触发 DOM 重画：1. 当前处于小范围 (Zoom > 5.5)，移动/缩放后视口边界变了，需要重新裁剪；2. 刚刚跨越了 5.5 的临界点（从全量切到裁剪，或从裁剪恢复全量）
...
function renderMapMarkers() {
    ...
    // 获取当前 Zoom 及视口边界（仅在 Zoom > 5.5 时计算视口）
    const currentZoom = map.getZoom();
    const enableCulling = currentZoom > 5.5;
